...
// Credit-card statements often list every charge as a positive "Amount" with no Dr/Cr
// column. A charge on a credit card is money OUT (spend), so for credit_card accounts we
// sign single-amount rows as negative, keeping obvious credits (payments to the card,
// cashback, refunds) positive. Split/indicator modes already carry the correct sign.
const CC_CREDIT = /(payment|received|cashback|rebate|refund|reversal)/i;

export function extractTransactions(headers, rows, mapping, accountType = 'bank_account') {
  ...
  for (const row of rows) {
    ...
    if (mapping.amountMode === 'split') {
      ...
      if (amount !== null && accountType === 'credit_card') {
        amount = CC_CREDIT.test(description) ? Math.abs(amount) : -Math.abs(amount);
      }
    }
